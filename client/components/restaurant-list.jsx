...
//   const [waitlist, setWaitlist] = useState(handleCurrentWait());
...
//   function modalClose() {
//     if (toggle && event.target.className === 'form-modal') {
//       setToggle(false);
//     }
//   }
...
//       }
//     };
//     fetch(`/api/restaurantId/${props.info.name}`, req)
//       .then(response => {
//         if (response.ok) {
...
//   function handleCurrentWait() {
//     fetch(`/api/restaurantId/${props.info.name}`)
//       .then(response => {
...
//         fetch(`/api/waitlist/${restaurantId}`)
...
//             {restaurant.categories.map(i => `${i.title}, `)}
//           </p>
//           <p style={{ margin: '5px 0px', fontSize: '12px' }}>
...
//           <button className={props.buttonDisplay} onClick={() => handleJoin()}>Join Waitlist</button>
//         </div>
//         <div onClick={() => this.modalClose()} className={this.state.toggle ? 'form-modal' : 'form-modal hidden'} >
//           <JoinForm name={restaurant.name} restaurantId={restaurantId} />
//         </div>
//       </>
